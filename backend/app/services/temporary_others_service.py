"""Others helper/service layer.

這份檔案已移除 mock_users / mock_labs / mock_orders / mock_storage_locations。
現在 Others 頁面會盡量讀正式 DB 資料；storage_locations 尚未建表時，會由 labs 自動產生：
- lab_name + 收樣區
- lab_name + 實驗暫存區
- lab_name + 待取件區
"""
...
master_data = {
    ...
    # 這裡不再放 Lab A / Lab B / Lab C，避免前端又拿假 lab_name 寫回資料庫。
    # 真正 lab 清單請從 GET /api/labs 或 GET /api/others 的 labs 欄位取得。
...
def removed_endpoint_response(name: str):
    raise HTTPException(
        status_code=410,
        detail=f"{name} 已移除 mock 資料，請改接正式 API 或正式資料表。",
    )
