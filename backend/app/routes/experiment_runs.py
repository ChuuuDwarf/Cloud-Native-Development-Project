...
Controller for the experiment-runs module — mounted from the central
``app/routes`` registry. Thin router; all logic lives in
:class:`app.modules.experiment_runs.service.ExperimentRunService`.
"""
...
@router.get("/{wip_id}/operators", response_model=ApiResponse[list[dict]])
async def list_operators(
    wip_id: str,
    service: Annotated[ExperimentRunService, Depends(get_experiment_run_service)],
    _: Annotated[CurrentUser, Depends(require_permission(EXPERIMENTS_OPERATE))],
) -> ApiResponse[list[dict]]:
    """此 WIP 所屬實驗室的人員/主管（給上機登記的操作人下拉）。"""
    return ApiResponse(data=await service.list_operators(wip_id))
...
@router.post("/{wip_id}/verify", response_model=ApiResponse[dict])
async def verify_data(
    ...
    """驗證原始數據完整性（待確認狀態），通過後才可確認結果。"""
    return ApiResponse(
        data=await service.verify_data(wip_id, body.operator),
        message="數據已驗證",
    )
