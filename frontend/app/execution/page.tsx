...
export default function ExecutionPage() {
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            EXPERIMENT EXECUTION · 上下機 / 進度 / 結果 / 中止
          </p>
        </div>
          ...
          <>
            ...
            <Btn
              ...
              onClick={() =>
                run(
                  () =>
                    experimentsApi.checkOut(w.wipId, {
                      operator: w.operator ?? "實驗室人員",
                    }),
                  "下機登記完成",
                )
              }
          ...
          >
            ...
            <Btn
              ...
              onClick={() =>
                run(
                  () =>
                    experimentsApi.confirm(w.wipId, { operator: "實驗室人員" }),
                  "結果已確認",
                )
              }
            >
  ...
  function promptProgress(w: Wip) {
    ...
    if (Number.isNaN(n) || n < 0 || n > 100) {
      setMsg({ text: "進度需為 0-100 的數字", ok: false });
      return;
    }
  ...
  function renderModal() {
    ...
    if (modal === "checkin")
      return (
        <CheckinModal w={target} run={run} onClose={() => setModal(null)} />
      );
    if (modal === "result")
      return (
        <ResultModal w={target} run={run} onClose={() => setModal(null)} />
      );
    if (modal === "abort")
      return <AbortModal w={target} run={run} onClose={() => setModal(null)} />;
    if (modal === "review")
      return (
        <ReviewModal w={target} run={run} onClose={() => setModal(null)} />
      );
    if (modal === "detail")
      return <DetailModal w={target} onClose={() => setModal(null)} />;
    return null;
  }
}
