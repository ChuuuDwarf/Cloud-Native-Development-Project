...
export default function Dashboard() {
  ...
  const blockedMachines = useMemo(
    () => dashboard?.machines.filter(machine => ['故障中', '保養中', '停用'].includes(machine.status)) ?? [],
    [dashboard],
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4, fontFamily: 'monospace' }}>
            {isGlobalView ? 'ALL LABS' : formatLab(dashboard?.scope)} · {dashboard?.user.role ?? '角色'} · {message}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 22 }}>
        <KpiCard label="待派工 WIP" value={dashboard?.kpis.pendingDispatches ?? 0} sub={isGlobalView ? '跨 LAB 待排' : '本實驗室待排'} color="var(--blue)" icon="📋" />
        <KpiCard label="排程中" value={dashboard?.kpis.schedulingDispatches ?? 0} sub="已產生系統預估" color="var(--cyan)" icon="🗂️" />
        <KpiCard label="不可用機台" value={dashboard?.kpis.blockedMachines ?? 0} sub="故障、保養或停用" color="var(--red)" icon="⚠️" />
        <KpiCard label="平均稼動率" value={`${dashboard?.kpis.avgUtilization ?? 0}%`} sub={`${dashboard?.kpis.machineCount ?? 0} 台機台`} color="var(--green)" icon="📈" />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={panelStyle}>
            <div style={panelHeaderStyle}>
              <span style={{ fontWeight: 700 }}>{isGlobalView ? '各 LAB 情況' : '本 LAB 情況'}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, padding: 16 }}>
              {(dashboard?.labs ?? []).map(lab => (
                <div key={lab.lab} style={labCardStyle}>
                  ...
                  <Metric label="不可用機台" value={lab.blockedMachineCount} danger={lab.blockedMachineCount > 0} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={panelStyle}>
            <div style={panelHeaderStyle}>
              ...
              <span style={badgeStyle}>{dashboard?.dispatches.length ?? 0} 筆</span>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--s2)' }}>
                  {['WIP', 'LAB', '實驗項目', '優先', '狀態', '機台', '預估時間'].map(header => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={panelStyle}>
            <div style={panelHeaderStyle}>
              ...
              <span style={badgeStyle}>{dashboard?.machines.length ?? 0} 台</span>
            </div>
            {(dashboard?.machines ?? []).slice(0, 8).map(machine => (
              <div key={machine.machineId} style={{ padding: '12px 16px', borderBottom: '1px solid var(--border2)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ fontFamily: 'monospace', color: 'var(--text)' }}>{machine.machineId}</span>
                  <span style={{ color: machine.status === '閒置' ? 'var(--green)' : machine.status === '使用中' ? 'var(--blue)' : 'var(--red)', fontSize: 11 }}>{machine.status}</span>
                </div>
                <div style={{ color: 'var(--text3)', fontSize: 11, marginTop: 3 }}>{formatLab(machine.lab)} · {machine.name}</div>
                <div style={{ height: 5, background: 'var(--s3)', borderRadius: 3, overflow: 'hidden', marginTop: 8 }}>
                  <div style={{ width: `${machine.utilization}%`, height: '100%', background: 'linear-gradient(90deg,var(--blue),var(--cyan))' }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={panelStyle}>
            ...
            {(blockedMachines.length ? blockedMachines : []).map(machine => (
              <div key={machine.machineId} style={{ padding: '12px 16px', borderBottom: '1px solid var(--border2)', fontSize: 12 }}>
                <strong style={{ color: 'var(--orange)' }}>{machine.machineId}</strong>
                <span style={{ color: 'var(--text2)' }}> · {formatLab(machine.lab)} · {machine.status}</span>
              </div>
            ))}
            {!blockedMachines.length && (
              <div style={{ padding: 16, color: 'var(--text3)', fontSize: 12 }}>目前沒有故障、保養或停用機台。</div>
            )}
          </div>
        </div>
      </div>
...
function Metric({ label, value, danger = false }: { label: string; value: number; danger?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text2)', fontSize: 12, marginTop: 6 }}>
      <span>{label}</span>
...
const labCardStyle = { background: 'var(--s2)', border: '1px solid var(--border2)', borderRadius: 8, padding: 14 }
