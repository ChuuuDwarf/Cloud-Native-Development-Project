...
describe("transferFlow 功能測試", () => {
  ...
  it("getRequestedExperiments 只解析 experiment_item，不解析 note", () => {
    expect(
      getRequestedExperiments({ ...sample, experiment_item: null, note: "Lab Z:不該被解析" })
    ).toEqual([]);
  });

  it("依 Lab 與實驗項目找出對應 WIP，並處理 null 欄位", () => {
    expect(
      findMatchingWipForExperiment(wips, { lab_name: "lab b", experiment_item: " 光學量測 " })
    ).toMatchObject({
      id: "wip-b",
    });

    expect(
      findMatchingWipForExperiment(wips, { lab_name: "Lab C", experiment_item: "XRD" })
    ).toBeNull();
  ...
  it("只有狀態 completed 的 WIP 視為實驗完成", () => {
    expect(isExperimentCompleted(wips, { lab_name: "Lab A", experiment_item: "SEM 觀察" })).toBe(
      true
    );
    expect(isExperimentCompleted(wips, { lab_name: "Lab B", experiment_item: "光學量測" })).toBe(
      false
    );
