...
describe('sampleDisplay 功能測試', () => {
  ...
  it('Lab 使用者看到已離開本 Lab 的樣品時，依交接狀態顯示遮罩狀態', () => {
    expect(getDisplaySampleStatus(transferredOutSample, labAUser, { ...transferBase, status: 'pending' })).toBe(
      'transfer_pending',
    )

    expect(getDisplaySampleStatus(transferredOutSample, labAUser, { ...transferBase, status: 'transferring' })).toBe(
      'transferred_waiting_receive',
    )

    expect(getDisplaySampleStatus(transferredOutSample, labAUser, { ...transferBase, status: 'received' })).toBe(
      'transferred_received',
    )

    expect(getDisplaySampleStatus(transferredOutSample, labAUser, { ...transferBase, status: 'cancelled' })).toBe(
      'cancelled',
    )
  })

  it('Lab 使用者看到已離開本 Lab 的樣品時，特殊樣品狀態仍顯示異常或取消', () => {
    expect(getDisplaySampleStatus(makeSample({ status: 'cancelled', current_location: 'Lab B' }), labAUser)).toBe(
      'cancelled',
    )

    expect(getDisplaySampleStatus(makeSample({ status: 'lost', current_location: 'Lab B' }), labAUser)).toBe('lost')

    expect(getDisplaySampleStatus(makeSample({ status: 'damaged', current_location: 'Lab B' }), labAUser)).toBe(
      'damaged',
    )
  ...
  it('Lab 使用者看到已離開本 Lab 的樣品時，依交接狀態顯示遮罩位置', () => {
    expect(getDisplaySampleLocation(transferredOutSample, labAUser, { ...transferBase, status: 'pending' })).toBe(
      '本實驗室交接待送區',
    )

    expect(getDisplaySampleLocation(transferredOutSample, labAUser, { ...transferBase, status: 'transferring' })).toBe(
      '已送出，等待接收實驗室（Lab B）收樣',
    )

    expect(getDisplaySampleLocation(transferredOutSample, labAUser, { ...transferBase, status: 'received' })).toBe(
      '已由接收實驗室（Lab B）收樣',
    )

    expect(getDisplaySampleLocation(transferredOutSample, labAUser, { ...transferBase, status: 'cancelled' })).toBe(
      '交接已取消',
    )
  })
  ...
  it('遮罩位置在沒有 to_lab 或特殊狀態時仍有可讀 fallback', () => {
    ...
    expect(getDisplaySampleLocation(makeSample({ status: 'cancelled', current_location: 'Lab B' }), labAUser)).toBe(
      '流程已取消',
    )

    expect(getDisplaySampleLocation(makeSample({ status: 'lost', current_location: 'Lab B' }), labAUser)).toBe(
      '樣品異常：遺失',
    )

    expect(getDisplaySampleLocation(makeSample({ status: 'damaged', current_location: 'Lab B' }), labAUser)).toBe(
      '樣品異常：破損',
    )
  ...
  it('依角色判斷樣品是否可見', () => {
    ...
    expect(isSampleVisibleForUser(makeSample({ applicant_name: '陳大華' }), factoryUser)).toBe(false)
  ...
  it('Lab 使用者可依目前 Lab、active、待取件、已取回篩選樣品', () => {
    ...
    expect(filterSamplesByView(samples, labAUser, 'picked_up').map((item) => item.id)).toEqual(['sample-5'])
    expect(filterSamplesByView(samples, labBUser, 'outbound').map((item) => item.id)).toEqual(['sample-4'])
