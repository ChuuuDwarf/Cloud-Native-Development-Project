...
function formatRequestedExperimentsLines(sample: Sample | null) {
  const displayText = formatRequestedExperiments(sample);
...
export default function WipPage() {
  ...
  function isSampleInLab(sample: Sample, labName: string) {
    const location = normalizeLocation(sample.current_location);
    const normalizedLab = normalizeLocation(labName);

    if (!location || !normalizedLab) return false;
  ...
  const activeSamples = useMemo(() => {
    return samples.filter((sample) => {
      if (!activeSampleStatuses.has(sample.status)) return false;

      // 樣品已交給下一個 Lab 後，current_location 會變成下一個 Lab 的位置。
      // 原 Lab 的 WIP 分貨頁不應該再看到它。
      if (!isSampleInCurrentLab(sample)) return false;
  ...
  const currentLabRequestedExperiments = useMemo(() => {
    return requestedExperiments.filter((item) => isSameLab(item.lab_name, currentLab));
  ...
  const selectedWips = useMemo(() => {
    ...
    return wips.filter((wip) => {
      return wip.sample_id === selectedSampleId && isSameLab(getWipLabName(wip), currentLab);
    });
  ...
  function resetFormsForSample(sample: Sample | null, lab: string, wipData: Wip[]) {
    const nextForms = makeAutoFormsForSample(sample, lab, wipData);
    const hasAutoGeneratedForms = nextForms.some((form) => form.auto_generated);
    ...
    setOpenSections((prev) => ({
      ...prev,
      sampleInfo: true,
      createWip: hasAutoGeneratedForms || shouldOpenCreateWipByDefault(sample),
      currentWips: true,
    }));
  }
  ...
  async function loadData() {
    ...
    try {
      ...
      const currentLabName = getCurrentLab(currentUser);
      ...
      const activeSampleData = sampleData.filter((sample) => {
        if (!activeSampleStatuses.has(sample.status)) return false;
        if (!isSampleInLab(sample, currentLabName)) return false;
      ...
      if (sampleIdFromUrl) {
        ...
        if (
          target &&
          activeSampleStatuses.has(target.status) &&
          isSampleInLab(target, currentLabName)
        ) {
          setSelectedSampleId(target.id);
          resetFormsForSample(target, currentLabName, wipData);
          return;
        }
        ...
        if (firstTargetSample) {
          ...
          resetFormsForSample(firstTargetSample, currentLabName, wipData);
          return;
        }

        setSelectedSampleId("");
        setForms([createEmptyWipForm(currentLabName)]);
        setOpenSections({
          sampleInfo: true,
          createWip: true,
          currentWips: true,
      ...
      if (currentSelected) {
        resetFormsForSample(currentSelected, currentLabName, wipData);
        return;
      }
  ...
  function validateForms() {
    ...
    if (!isSampleInCurrentLab(selectedSample)) {
      return "此樣品目前不在本 Lab，不能在本 Lab 分貨";
    }
          ...
          <section style={leftPanelStyle}>
            <div style={panelHeaderStyle}>
              <div>
                ...
                <div style={panelHintStyle}>只顯示目前位置在本 Lab、且可建立 WIP 的樣品</div>
              </div>
            ...
            <div style={sampleListStyle}>
              {activeSamples.map((sample) => {
                ...
                const currentLabCount = sampleRequestedExperiments.filter((item) =>
                  isSameLab(item.lab_name, currentLab)
                ).length;
              ...
              <>
                ...
                >
                  <div style={detailGridStyle}>
                    ...
                    <InfoItem
                      label="全部實驗需求"
                      value={formatRequestedExperimentsLines(selectedSample)}
                    />
