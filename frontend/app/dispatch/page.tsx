...
export default function DispatchPage() {
  ...
  const loadData = useCallback((user?: AppUser) => {
    Promise.all([
      fetch(`${apiUrl}/api/machines`, {
        ...
        res.ok ? res.json() : Promise.reject(new Error("machines failed")),
      ),
      fetch(`${apiUrl}/api/recipes`, {
        headers: authHeaders(user?.userId),
      }).then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("recipes failed")),
      ),
      fetch(`${apiUrl}/api/dispatches`, {
        headers: authHeaders(user?.userId),
      }).then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("dispatches failed")),
      ),
    ])
      .then(
        ([machinePayload, recipePayload, dispatchPayload]: [
          { data: Machine[] },
          { data: Recipe[] },
          { data: Dispatch[] },
        ]) => {
          ...
          setActiveDispatchId(
            (current) => current || dispatchPayload.data[0]?.dispatchId || "",
          );
  ...
  function createDispatch() {
    fetch(`${apiUrl}/api/dispatches`, {
      ...
      .catch(() =>
        setMessage(
          "新增待派工 WIP 失敗，只有實驗室人員可新增，並請確認 dispatch ID 不重複",
        ),
      );
  }
  ...
  function suggestSchedule() {
    fetch(`${apiUrl}/api/dispatches/suggest`, {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("suggest failed")),
      )
      .then(() => loadData())
      .catch(() =>
        setMessage("產生建議失敗，廠區使用者不可操作，或機台與 WIP 無法對應"),
      );
  }
  ...
  function replan(reason: string, recommendedStrategy: Strategy) {
    ...
    fetch(`${apiUrl}/api/dispatches/replan`, {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("replan failed")),
      )
      .then(() => loadData())
  ...
  function assignDispatch() {
    ...
    fetch(`${apiUrl}/api/dispatches/${activeDispatch.dispatchId}/assign`, {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject(new Error("assign failed")),
      )
      .then(() => loadData())
      .catch(() =>
        setMessage(
          "確認派工失敗，只有實驗室人員可派工，並請確認機台狀態與 Recipe 相容",
        ),
      );
  }
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={suggestSchedule} style={buttonStyle}>
            產生建議
          </button>
        </div>
      </div>
      ...
      >
        ...
        {replanPolicies.map((policy) => (
          <button
            ...
            onClick={() => replan(policy.reason, policy.strategy)}
      ...
      >
        <div style={panelStyle}>
          <div style={panelHeaderStyle}>
            ...
            <button
              onClick={() => setForm(demoDispatchForm)}
              style={smallButtonStyle}
            >
              快速填入
            </button>
          ...
          >
            <input
              ...
              onChange={(event) =>
                setForm({ ...form, dispatchId: event.target.value })
              }
              style={inputStyle}
            />
            <input
              ...
              value={form.wipId}
              onChange={(event) =>
                setForm({ ...form, wipId: event.target.value })
              }
              style={inputStyle}
            />
            <input
            ...
            <select
              ...
              onChange={(event) =>
                setForm({ ...form, experimentItem: event.target.value })
              }
              style={inputStyle}
            >
            ...
            <input
              ...
              style={inputStyle}
            />
            <button onClick={createDispatch} style={buttonStyle}>
              新增 WIP
            </button>
          </div>
        </div>
            ...
            <tbody>
              {dispatches.map((dispatch) => (
                <tr
                  ...
                  onClick={() => setActiveDispatchId(dispatch.dispatchId)}
                  style={{
          ...
          >
            ...
            >
              <input
                ...
                onChange={(event) =>
                  updateScheduleDraft("scheduledStart", event.target.value)
                }
                style={inputStyle}
              />
              <input
                ...
                value={scheduledEnd}
                onChange={(event) =>
                  updateScheduleDraft("scheduledEnd", event.target.value)
                }
                style={inputStyle}
              />
            </div>
            ...
            <button
              disabled={!selectedMachineId || !assignableRecipes[0]}
              onClick={assignDispatch}
              style={{
                ...buttonStyle,
                opacity: selectedMachineId && assignableRecipes[0] ? 1 : 0.45,
                cursor:
                  selectedMachineId && assignableRecipes[0]
                    ? "pointer"
                    : "not-allowed",
              }}
            >
