...
const statusTypes: Record<MachineStatus, 'idle' | 'running' | 'pending' | 'rejected'> = {
  閒置: 'idle',
  使用中: 'running',
  保養中: 'pending',
  故障中: 'rejected',
...
export default function MachinePage() {
  ...
  const summary = useMemo(() => {
    const available = machines.filter(machine => machine.status === '閒置').length
    const blocked = machines.filter(machine => ['故障中', '保養中', '停用'].includes(machine.status)).length
    const avg = machines.length
      ? Math.round(machines.reduce((sum, machine) => sum + machine.utilization, 0) / machines.length)
      : 0
    return { available, blocked, avg }
  }, [machines])

  function createMachine() {
    const existingMachine = machines.find(machine => machine.machineId === form.machineId)
    fetch(`${apiUrl}/api/machines${existingMachine ? `/${form.machineId}` : ''}`, {
      ...
      body: JSON.stringify({
        ...
        name: form.name,
        lab: form.lab,
        supportedItems: form.supportedItems.split(',').map(item => item.trim()).filter(Boolean),
        owner: form.owner,
      ...
      .then(() => {
        setForm({ machineId: '', name: '', lab: '', supportedItems: '', owner: '', utilization: '0', lastMaintenance: '尚未保養' })
        loadMachines()
      })
      .catch(() => setMessage('儲存機台失敗，請確認使用者權限、ID 不重複且後端已啟動'))
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 16 }}>
        <div style={panelStyle}>
          <div style={panelHeaderStyle}>
            ...
            <button onClick={() => setForm(demoMachineForm)} style={smallButtonStyle}>快速填入</button>
          </div>
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <input placeholder="上次保養日" value={form.lastMaintenance} onChange={event => setForm({ ...form, lastMaintenance: event.target.value })} style={inputStyle} />
            <button onClick={createMachine} style={buttonStyle}>{machines.some(machine => machine.machineId === form.machineId) ? '儲存編輯' : '新增機台'}</button>
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--s2)' }}>
                {['機台', '實驗室', '狀態', '支援項目', '稼動率', '保養日', '操作'].map(header => (
            ...
            <tbody>
              {machines.map(machine => (
                <tr key={machine.machineId} style={{ borderBottom: '1px solid var(--border2)' }}>
                  ...
                  <td style={tdStyle}>
                    <button onClick={() => editMachine(machine)} style={smallButtonStyle}>編輯</button>{' '}
                    <button onClick={() => updateStatus(machine.machineId)} style={buttonStyle}>套用狀態</button>
                  </td>
                </tr>
              ))}
              {!machines.length && (
                <tr><td colSpan={7} style={{ ...tdStyle, textAlign: 'center', padding: 28 }}>尚無機台，請先從左側新增。</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
const buttonStyle = { background: 'var(--blue)', border: '1px solid var(--border)', color: '#fff', padding: '7px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer' }
